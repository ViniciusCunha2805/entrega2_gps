...
import { MONTHS_PT, DAYS_PT, TIMES } from "../../models/initialState.js";

export default function CBooking({ TopBar, Nav, setTab }) {
  const { state, dispatch } = useApp();

  const [selSvcName, setSelSvcName] = useState(state.services[0]?.name || "");
  ...
  const selSvc =
    state.services.find((s) => s.name === selSvcName) ||
    state.services[0] ||
    {};
  ...
  const scheduleMatch =
    selDay !== null && selTime
      ? state.serviceSchedules.find(
          (rule) =>
            rule.service === selSvc.name &&
            rule.day === selDay &&
            rule.time === selTime,
        )
      : null;
  const activePrice = scheduleMatch ? scheduleMatch.price : selSvc.price;
  const activeDur = scheduleMatch ? scheduleMatch.dur : selSvc.dur;

  function confirmBooking() {
    if (!selDate) {
      dispatch({ type: "TOAST", msg: "Escolha uma data" });
      return;
    }
    if (!selTime) {
      dispatch({ type: "TOAST", msg: "Escolha um horário" });
      return;
    }
  ...
  for (let d = 1; d <= total; d++) {
    ...
    const hasDot = state.appointments.some(
      (a) =>
        a.date === ds &&
        a.status !== "cancelled" &&
        a.clientEmail === state.currentUser.email,
    );
  ...
  return (
    <div className="screen active">
      ...
      <div className="scroll-area">
        {/* Serviços */}
        <p className="sec-title" style={{ marginTop: 0 }}>
          Escolha o serviço
        </p>
        {state.services.map((s) => (
          <div
            ...
            onClick={() => setSelSvcName(s.name)}
          >
            <div>
              <div className="service-option-name">{s.name}</div>
              <div className="service-option-sub">
                {s.icon} {s.dur}
              </div>
            </div>
            ...
            {cells.map((c, i) => (
              <div
                ...
                className={[
                  ...
                  .join(" ")}
                onClick={() =>
                  c.type === "cur" && !c.isPast && setSelDate(c.ds)
                }
              >
                {c.day}
              </div>
        ...
        {scheduleMatch && (
          ...
          >
            <div
              style={{ fontSize: 13, color: "var(--gold)", marginBottom: 4 }}
            >
            ...
            >
              <div style={{ fontSize: 14, fontWeight: 600 }}>
                R$ {scheduleMatch.price}
              </div>
              <div style={{ fontSize: 12, color: "var(--text2)" }}>
                {selSvc.name} • {scheduleMatch.dur}
              </div>
            </div>
          </div>
        )}
